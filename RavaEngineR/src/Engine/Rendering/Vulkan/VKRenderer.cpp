...
//
 void Renderer::RenderpassEntities(/*entt::registry& registry, Rava::Camera& currentCamera*/) {
	if (_currentCommandBuffer) {
		GlobalUbo ubo{};
		//ubo.Projection  = RV::Engine::Instance->GetCurrentCamera().GetProjection();
		//ubo.View        = RV::Engine::Instance->GetCurrentCamera().GetView();
		//ubo.InverseView = RV::Engine::Instance->GetCurrentCamera().GetInverseView();
		//ubo.Gamma       = RV::Engine::Instance->GetGamma();
		//ubo.Exposure    = RV::Engine::Instance->GetExposure();

		////_pointLightRenderSystem->Update(_currentCommandBuffer, ubo, RV::Engine::Instance->GetCurrentScene().GetRegistry());
...
void Renderer::Begin3DRenderPass() {
